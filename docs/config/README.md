...
| `osc_133`         | `false`                        | Emit [OSC 133](https://gitlab.freedesktop.org/Per_Bothner/specifications/-/blob/master/proposals/semantic-prompts.md) semantic-prompt escape sequences (A/B/D from the prompt, C from preexec on bash/zsh/fish). |
...
> only emitted for `bash`, `zsh`, and `fish` - other shells still get
> `A`/`B`/`D` from the prompt but no preexec marker.
